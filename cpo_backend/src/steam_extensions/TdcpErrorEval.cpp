...
Eigen::Matrix<double, 1, 1> TdcpErrorEval::evaluate() const {
  ...
  EvalTreeHandle<lgmath::se3::Transformation>
      blkAutoEvalTransform = T_ag_->getBlockAutomaticEvaluation();
  const lgmath::se3::Transformation &T_ag = blkAutoEvalTransform.getValue();
  EvalTreeHandle<Eigen::Vector3d>
      blkAutoEvalPosition = r_ba_ina_->getBlockAutomaticEvaluation();
...
Eigen::Matrix<double, 1, 1> TdcpErrorEval::evaluate(
  ...
  if (r_ba_ina_->isActive()) {
    ...
    Eigen::Matrix<double, 1, 3>
        J_1 = u_a21_.transpose() * T_ag.C_ba().transpose(); // u^T * C_ag^T

    Eigen::Matrix<double, 1, 3> newLhs = lhs * J_1;
    r_ba_ina_->appendBlockAutomaticJacobians(newLhs,
                                             blkAutoEvalPosition.getRoot(),
                                             jacs);
  }
  ...
  if (T_ag_->isActive()) {
    ...
    // Get Jacobians
    Eigen::Matrix<double, 1, 3>
        J_2 = u_a21_.transpose() * T_ag.C_ba().transpose() * r_hat;

    Eigen::Matrix<double, 1, 6> newLhs;
    newLhs << 0, 0, 0, lhs * J_2;
    T_ag_->appendBlockAutomaticJacobians(newLhs,
                                         blkAutoEvalTransform.getRoot(),
                                         jacs);
  }
